...
function selectPreguntasFiltradas($titulo, $page, $fechaInicio = "", $fechaFin = "", $tags = "")
{
    $dbh = connect();
    $offset = 8 * ($page - 1);
    //Si fecha inicio o fecha fin vienen vacias, se le asigna unas fechas por defecto
    $fechaInicio = $fechaInicio == "" ? "1999-01-01" : $fechaInicio;
    $fechaFin = $fechaFin == "" ? "2050-01-01" : $fechaFin;
    //Utilizamos el CONCAT para que si el titulo o tags vienen vacios, no afecte a la consulta, por eso ponemos el %
    $stmt = $dbh->prepare("SELECT * FROM pregunta WHERE titulo LIKE CONCAT('%', :titulo, '%') AND tags  LIKE CONCAT('%', :tags, '%') 
                            AND fecha  BETWEEN :fechaInicio AND :fechaFin  ORDER BY id DESC LIMIT 8 OFFSET :offset");
